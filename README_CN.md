...
| 100+     | Aho-Corasick  | 自动机 O(n) 扫描，与词数无关        |

Regex 不再在任何规模下被自动选中——在 2.7 万词条的词典上，超长 alternation
正则的扫描比 Aho-Corasick 慢约 6 万倍——但仍可通过
`Filter::with_algorithm(MatchAlgorithm::Regex)` 或 CLI 的 `--algorithm regex`
强制使用（其编译大小上限已提升至 64 MB，超大词库也能正常编译）。
